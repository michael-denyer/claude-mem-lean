...
2. Queries SQLite for recent session summaries (last 5)
3. Queries SQLite for recent observations (configurable, default 25)
...
**Output format:**
```markdown
# [claude-mem] recent context, 2026-09-16 9:43pm BST
Mode: Code Development (code)

### Sep 16, 2026
26982 9:28p ✅ Published Consolidated PR #182, Retargeted #184, Closed #183
26990 9:43p ✅ PR #184 Review Requested from wisemdaya
S5625 Ship the PR stack (Sep 16, 2026)

**Next Steps**: Wait for CI on #184, then merge.
```

The agent copy carries no legend, stats line, or fetch instructions. The glyphs read in context and the search tools are already in the model's tool list. Observations whose titles repeat within an hour (sharing most of their words) collapse to the newest row. The human copy (`claude-mem context`) shows the title line and the timeline, with no mode line, legend, or token stats.
